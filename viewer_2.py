...
class HabitatSimInteractiveViewer(Application):
    ...
    def move_and_look(self, repetitions: int) -> None:
        ...
        action_queue=['move_backward']
        point_nav=[0., 0.11937292, 2.5200002 ]
        tolerance = 1e-3
        repetitions=1
        if np.allclose(agent.state.position, point_nav, atol = tolerance):
            action_queue=['turn_right']
        for _ in range(int(repetitions)):
            # print("repetitions",repetitions) 动作列表循环执行的次数，一般为1,偶尔为2，影响的是动作执行的速度
            # print(action_queue) # action_queue即为agent的动作指令
            # action有‘move_forward','move_backward','move_left',move_right','turn_right','turn_left'等
            [agent.act(x) for x in action_queue] # 执行动作列表
            print(agent.state.position) # 返回agent的坐标
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--scene",
        default="/home/www-ubuntu/habitat/habitat-sim/data/HM3Dexample/hm3d-example-glb-v0.2/00337-CFVBbU9Rsyb/CFVBbU9Rsyb.glb",
        type=str,
        help='scene/stage file to load (default: "./data/test_assets/scenes/simple_room.glb")',
    )
    parser.add_argument(
        "--dataset",
        default="/home/www-ubuntu/habitat/habitat-sim/data/HM3Dexample/hm3d-example-configs/hm3d_example_basis.scene_dataset_config.json",
        type=str,
        metavar="DATASET",
